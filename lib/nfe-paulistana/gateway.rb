...
module NfePaulistana
  class Gateway
    ...
    def get_client
      Savon.client(soap_version: 1, env_namespace: :soap, ssl_verify_mode: :peer, ssl_cert_file: @options[:ssl_cert_path], ssl_cert_key_file: @options[:ssl_key_path], ssl_cert_key_password: @options[:ssl_cert_pass], wsdl: @options[:wsdl], namespace_identifier: nil)
    end
  end
end
